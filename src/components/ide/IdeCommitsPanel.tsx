...
export function IdeCommitsPanel({ commits, loading, onLoadCommitDetail }: Props) {
  ...
  const toggleCommit = async (sha: string) => {
    if (expandedSha === sha) { setExpandedSha(null); setDetail(null); setSelectedFile(null); return; }
    setExpandedSha(sha); setSelectedFile(null);
    setLoadingDetail(true);
    try { setDetail(await onLoadCommitDetail(sha)); }
          ...
          <div key={c.sha}>
            <button onClick={() => toggleCommit(c.sha)}
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="text-xs text-white font-medium truncate flex-1">{c.commit.message.split("\n")[0]}</p>
              </div>
              <div className="flex items-center gap-2 mt-1 ml-4">
                {c.author?.avatar_url && <img src={c.author.avatar_url} className="w-4 h-4 rounded-full" alt="" />}
                <span className="text-[10px] text-white/30">{c.author?.login || c.commit.author.name}</span>
                <span className="text-[10px] text-white/20 font-mono">{c.sha.slice(0, 7)}</span>
                <span className="text-[10px] text-white/15 ml-auto">{new Date(c.commit.author.date).toLocaleDateString()}</span>
              </div>
              {/* Full commit message (multi-line) */}
              {expandedSha === c.sha && c.commit.message.includes("\n") && (
                <pre className="mt-2 ml-4 text-[10px] text-white/40 whitespace-pre-wrap">{c.commit.message.split("\n").slice(1).join("\n").trim()}</pre>
              )}
            </button>

            {expandedSha === c.sha && (
              <div className="bg-[#060606] border-b border-white/5">
                {loadingDetail && <div className="flex justify-center py-4"><Loader2 size={14} className="animate-spin text-blue-400" /></div>}
                {detail && detail.stats && (
                ...
                {detail?.files?.map(f => (
                  <div key={f.filename} className="border-b border-white/[0.03]">
                    <button onClick={() => setSelectedFile(selectedFile === f.filename ? null : f.filename)}
                      className="w-full text-left px-3 py-1.5 flex items-center gap-2 text-[10px] hover:bg-white/[0.02]">
                      {selectedFile === f.filename ? <ChevronDown size={10} className="text-white/30" /> : <ChevronRight size={10} className="text-white/30" />}
                      ...
                      <span className="text-white/50 truncate flex-1">{f.filename}</span>
                      <span className="text-green-400">+{f.additions}</span>
                      <span className="text-red-400">−{f.deletions}</span>
                    </button>
                    {(selectedFile === f.filename) && f.patch && (
                      <pre className="px-1 pb-1 text-[9px] font-mono overflow-x-auto max-h-[400px]" style={{ scrollbarWidth: "thin" }}>
                        {f.patch.split("\n").map((line, i) => (
