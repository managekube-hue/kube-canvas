...
import type { ReachMilestone } from "@/hooks/useReachMilestones";

interface Props {
  milestones: ReachMilestone[];
  loading: boolean;
  onCreateMilestone: (title: string, description: string, dueDate?: string) => Promise<void>;
  onUpdateMilestone: (id: string, updates: { status?: string }) => Promise<void>;
}
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "thin" }}>
        ...
        {filtered.map(m => (
          <div key={m.id} className="px-3 py-3 border-b border-white/5">
            <div className="flex items-center gap-2">
              {m.status === "open" ? <Clock size={12} className="text-blue-400" /> : <CheckCircle2 size={12} className="text-green-400" />}
              <span className="text-xs font-semibold text-white flex-1">{m.title}</span>
              {m.status === "open" ? (
                <button onClick={() => onUpdateMilestone(m.id, { status: "closed" })}
                  className="text-[9px] text-white/30 hover:text-white/60 flex items-center gap-1">
                  <CheckCircle2 size={9} /> Close
                </button>
              ) : (
                <button onClick={() => onUpdateMilestone(m.id, { status: "open" })}
