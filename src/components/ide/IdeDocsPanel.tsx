...
import { BookOpen, ChevronRight, ChevronDown, FileText, Loader2, ArrowLeft, ExternalLink } from "lucide-react";
...
export function IdeDocsPanel({ tree, onLoadFile, onOpenInEditor }: Props) {
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="px-3 py-2 border-b border-white/5 flex items-center gap-2">
          <button onClick={() => setSelectedDoc(null)} className="text-white/30 hover:text-white/60">
            <ArrowLeft size={14} />
          </button>
          <BookOpen size={12} className="text-emerald-400" />
          <span className="text-[10px] text-white/50 truncate flex-1">{selectedDoc}</span>
          <button onClick={() => onOpenInEditor(selectedDoc)} className="text-white/30 hover:text-blue-400" title="Open in editor">
            <ExternalLink size={12} />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto px-4 py-4" style={{ scrollbarWidth: "thin", scrollbarColor: "#1e293b transparent" }}>
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-3 py-2 border-b border-white/5 flex items-center gap-2">
        <BookOpen size={12} className="text-emerald-400" />
        <span className="text-[10px] text-white/40 font-bold uppercase tracking-widest">Documentation</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-1" style={{ scrollbarWidth: "thin", scrollbarColor: "#1e293b transparent" }}>
        ...
        {!loading && docTree.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12 text-white/20">
            ...
            <p className="text-[10px] text-white/15 mt-1">Add .md files to your repo</p>
          </div>
        )}
